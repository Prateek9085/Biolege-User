...
class FadeInDTU extends StatelessWidget {
  final double delay;
  final Widget child;
  final Curve curve = Curves.easeOutCubic;

  FadeInDTU(this.delay, this.child);
...
class FadeInRTL extends StatelessWidget {
  final double delay;
  final Widget child;
  final Curve curve = Curves.easeOutCubic;

  FadeInRTL(this.delay, this.child);

  @override
  Widget build(BuildContext context) {
    final tween = MultiTween<AniProps>()
      ..add(AniProps.opacity, 0.0.tweenTo(1.0), 500.milliseconds, curve)
      ..add(AniProps.translateX, 0.0.tweenTo(30.0), 500.milliseconds, curve);
